...
// src/repositories/dynamodb.ts
var DynamoRepo = class {
  constructor() {
    this.DB = new import_aws_sdk.DynamoDB({
      region: "us-east-1",
  ...
  async findItemByUserId(userId) {
    ...
    return (_a = result.Items) == null ? void 0 : _a.map((item) => {
      return import_aws_sdk.DynamoDB.Converter.unmarshall(item);
    });
  }
  async listAll() {
  ...
  async create({ userId, title, done, deadline }) {
    const id = v4();
...
async function createTodoHandler(event) {
  const BAD_REQUEST_MESSAGE = "Os campos necess\xE1rios n\xE3o foram passados.";
  try {
    ...
    if (!title || typeof done !== "boolean" || !deadline || !userId)
      throw new Error(BAD_REQUEST_MESSAGE);
    ...
    return {
      ...
      body: JSON.stringify({
        error: e.message == BAD_REQUEST_MESSAGE ? BAD_REQUEST_MESSAGE : "Todo n\xE3o encontrado."
      })
    };
  }
}
async function listTodoByUserIdHandler(event) {
  try {
    const { userId } = event.pathParameters;
    const dynamoClient = new DynamoRepo();
...
var handler = async (event) => {
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({
      error: "M\xE9todo Http recusado. \xC9 permitido apenas m\xE9todos GET e POST."
    })
  };
};
